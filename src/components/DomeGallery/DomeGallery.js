...
export default function DomeGallery({
    ...
    useEffect(() => {
        ...
        const loop = (time) => {
            ...
            if (!draggingRef.current && !inertiaRAF.current && !openingRef.current && !focusedElRef.current) {
                const nextY = wrapAngleSigned(rotationRef.current.y + autoRotationSpeed * (dt / 16));
                rotationRef.current = { ...rotationRef.current, y: nextY };
                applyTransform(rotationRef.current.x, nextY);
            }
            autoRotationRAF.current = requestAnimationFrame(loop);
        };
        autoRotationRAF.current = requestAnimationFrame(loop);

        return () => {
            if (autoRotationRAF.current) cancelAnimationFrame(autoRotationRAF.current);
        };
    }, [autoRotationSpeed]);
